...
{
    public class ExpedientePruebasServices : IServices
    {
        private readonly HttpClient _httpClient;
        private readonly IConfiguration _configuration;
        private readonly string _basePath = "F2_PreparacionFabricacion/";

        public ExpedientePruebasServices() 
        {
       ...
       public async Task<HttpResponseMessage> Leer()
        ...
        public async Task<HttpResponseMessage> Leer(int pageNumber, int pageSize)
        {
            var response = await _httpClient.GetAsync($"ExpedientePruebas?pageNumber={pageNumber.ToString()}&pageSize={pageSize.ToString()}");
            return response;
        }
