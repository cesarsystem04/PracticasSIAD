...
{
    public class OrdenFabricacionServices : IServices
    {
        private readonly HttpClient _httpClient;
        private readonly IConfiguration _configuration;
        private readonly string _basePath = "F2_PreparacionFabricacion/";

        public OrdenFabricacionServices() 
        {
       ...
       public async Task<HttpResponseMessage> Leer()
        {
            var response = await _httpClient.GetAsync("OrdenFabricacion");
            return response;
        }


        public async Task<HttpResponseMessage> Leer(string idOrdenFabricacion)
        {
            var response = await _httpClient.GetAsync($"OrdenFabricacion/{idOrdenFabricacion}");
            return response;
        }
